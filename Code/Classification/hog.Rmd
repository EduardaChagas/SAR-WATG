---
title: "Classification of Regions with HOG"
...
In this script, we will evaluate the performance of the HOG technique for region classification in PolSAR textures.
...
hog.data = read.csv(file="../../Data/HOG.csv", header=TRUE, sep=",")

split = 0.85
trainIndex = createDataPartition(regions, p = split, list = FALSE)

x = data.frame(hog.data[trainIndex, ])
y = factor(regions[trainIndex])

x_validation = data.frame(hog.data[-trainIndex, ])
y_validation = factor(regions[-trainIndex])

hog = data.frame(hog.data[trainIndex, ], "regions" = regions[trainIndex])
```
...
knnFit = train(regions~., data = hog, method = "knn", 
               trControl = ctrl, 
