...
transition.graph.weight.analysis <- function(tal){
  ...
  for(i in n){
    ...
    for(j in c(1:ns.guatemala)){
      ...
      g = transition.graph.weight(ts, i, tal)
    ...
    write.csv(Entropy.Complexity, paste('../Data/EntropyComplexityWATGD', i, 'T', tal, '.csv', sep = ""), row.names = FALSE)
  }
}

WATG.parallel <- function(){
  tal = c(1,2,3,4,5) #Delay parameter
  registerDoParallel(cores = 5)
  for(i in tal){
    transition.graph.weight.analysis(i)
  }
}
