...
\end{tcolorbox}
\todo[inline]{Estamos trabalhando nisso}
...
The results presented were based on samples organized as follows: 40 samples from the forest regions of Guatemala; 80 samples from the oceanic regions of Cape Canaveral, divided into two types with different contrasts; and 40 samples from urban areas in the city of Munich. Thus, the results are shown in Fig. 7 and Table I configure a set of samples with both types of contrast.
...
As reported in section III-A, the data set that we built for conducting the experiments have only $160$ samples.
...
\end{tcolorbox}
\todo[inline]{Precisa ser atualizado após os novos experimentos}
...
In the experiments carried out to analyze homogeneous textures, among the weighting techniques presented, the ones that produced the worst performance were WPE and AAPE.
These results can be explained by the weighting method applied, as both algorithms apply the parameter referring to the amplitude under the histogram of ordinal patterns.
On the other hand, FGPE creates a new set of symbols by adding to the end of each ordinal pattern a parameter referring to its amplitude, managing to increase the granularity of the analysis, and improving performance.
The best results, in general, are obtained by WATG, since it weighs the amplitude using no longer the pattern histogram, but using the transition graph.
