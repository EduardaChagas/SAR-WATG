...
\begin{document}
	...
	Line~\ref{Line:Peano} transforms the input texture patch $P$ in a 1-D signal with a Hilbert-Peano sequence.
	%
	...
	We perform a data dimensionality reduction by turning the 2-D patch into a 1-D signal.
	...
	Using the Hilbert-Peano curve, we reduce the data dimensionality while maintaining the spatial dependence information of the patch.
	...
	The first step of the \texttt{WATG} subroutine is to calculate the ordinal patterns of the 1-D signal by Band-Pompe symbolization.
	...
	Let $\mathbbm{P} = \{p_{(\widetilde\pi^D_1, \widetilde\pi^D_1)}, p_{(\widetilde\pi^D_1, \widetilde\pi^D_2)}, \dots, p_{(\widetilde\pi^D_{D!}, \widetilde\pi^D_{D!})} \} = \{p_1,\dots,p_{D!^2}\}$ be the probability function obtained from the 1-D signal weighted amplitude transition graph $\mathbbm{X}$.
	...
	The speckle noise was modeled as outcomes of independent, identically distributed unitary-mean Gamma random variables with shape parameter $L$ (the number of looks, which controls the signa-to-noise ratio)
	$W(L) \sim \Gamma(L, L)
	$,
	...
	%%% Horror!!! Por que não usou subfigure?
	%%% Alterado 
	\begin{figure}
		\centering
		\subfigure[$z$]{\label{fig:speckle1}\includegraphics[width = 28mm]{Figures/z.png}}
		\subfigure[$I(100)$]{\label{fig:speckle2}\includegraphics[width = 28mm]{Figures/z100.png}}
		\subfigure[$I^T(500)$]{\label{fig:speckle3}\includegraphics[width = 28mm]{Figures/z500t.png}}
	...
	\begin{itemize}
		...
		It was $8917 \times 3300$ pixels and its image resolution is 
		\SI[inter-unit-product =$\cdot$]{10}{\meter} $\times$ \SI[inter-unit-product =$\cdot$]{2}{\meter};
		%%% ACF Usar \SI para digitar unidades
		%%% ETCC Done
		\item Cape Canaveral Ocean Regions (acquired on September 22, 2016).
		It was $7038 \times 3300$ pixels and its image resolution is  
		\SI[inter-unit-product =$\cdot$]{10}{\meter} $\times$ \SI[inter-unit-product =$\cdot$]{2}{\meter};
		...
		It was $5773 \times 3300$ pixels and its image resolution is  
		\SI[inter-unit-product =$\cdot$]{10}{\meter} $\times$ \SI[inter-unit-product =$\cdot$]{3}{\meter}.
	\end{itemize}
	...
	\begin{figure*}[hbt]
		\begin{tabular}{ccccc}
			\includegraphics[width=0.175\textwidth]{Figures/guatemalaflorest.pdf} &   \includegraphics[width=0.175\textwidth]{Figures/Cape1.pdf} &
			\includegraphics[width=0.175\textwidth]{Figures/Cape2.pdf} &  \includegraphics[width=0.175\textwidth]{Figures/munichUrban.pdf} &  \includegraphics[width=0.175\textwidth]{Figures/pasture.pdf} \\
		\end{tabular}
