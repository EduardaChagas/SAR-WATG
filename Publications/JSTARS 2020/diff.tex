...
%DIF DEL SARTexture-IT.R0.tex   Wed Aug 26 03:33:10 2020
%DIF ADD SARTexture-IT.R1.tex   Wed Aug 26 04:14:23 2020
...
\DIFdelbegin %DIFDELCMD < \IEEEPARstart{T}{exture} %%%
\DIFdelend \DIFaddbegin \IEEEPARstart{T}\DIFadd{exture }\DIFaddend is an elusive trait.
...
\begin{equation}
H(\DIFdelbegin \DIFdel{\mathbb{P}}\DIFdelend \DIFaddbegin \DIFadd{\mathbbm{P}}\DIFaddend ) = -\frac{1}{2\log D!}\sum_{\ell=1}^{D!^2} p_{\ell} \log p_{\ell} .
\label{eq:Entropia}
\end{equation}
...
\DIFdelbegin \DIFdel{The proposed method was evaluated based on }\DIFdelend \DIFaddbegin \DIFadd{We used the HH backscatter magnitudes of }\DIFaddend three quad-polarimetric L-band SAR images from the NASA Jet Propulsion Laboratory’s (JPL’s) uninhabited aerial vehicle synthetic aperture radar (UAVSAR) \DIFdelbegin \DIFdel{.
We used the HH backscatter magnitudes of}\DIFdelend \DIFaddbegin \DIFadd{sensor with $L=36$ nominal looks}\DIFaddend :
\begin{itemize}
	\item \DIFaddbegin \DIFadd{forest and pasture region of }\DIFaddend Sierra del Lacandón National Park, \DIFdelbegin \DIFdel{forest region in Guatemala}\DIFdelend \DIFaddbegin \DIFadd{Guatemala, }\DIFaddend (acquired on April 10, 2015)\footnote{\protect{\url{https://uavsar.jpl.nasa.gov/cgi-bin/product.pl?jobName=Lacand_30202_15043_006_150410_L090_CX_01\#dados}}}\DIFdelbegin \DIFdel{;
	}\DIFdelend \DIFaddbegin \DIFadd{. 
...
We \DIFdelbegin \DIFdel{assess }\DIFdelend \DIFaddbegin \DIFadd{assessed }\DIFaddend the effectiveness of each approach using the following metrics: 
\DIFdelbegin \DIFdel{recall }\DIFdelend \DIFaddbegin \begin{itemize}
	\item \DIFadd{Recall }\DIFaddend or True Positive Rate (TPR)\DIFdelbegin \DIFdel{, precision }\DIFdelend \DIFaddbegin \DIFadd{: 
	}\begin{equation*}
	\DIFadd{\text{TPR} = \frac{\text{True Positive}}{\text{True Positive} + \text{False Negative}}.
	}\end{equation*}
	%DIF > 
	\item \DIFadd{Precision }\DIFaddend or Positive Predictive Value (PPV)\DIFdelbegin \DIFdel{, Overall Accuracy (OA), and }\DIFdelend \DIFaddbegin \DIFadd{: }\begin{equation*}
	\DIFadd{\text{PPV} = \frac{\text{True Positive}}{\text{True Positive} + \text{False Positive}}.
	}\end{equation*}
	%DIF > 
	...
	\DIFadd{\text{AA} = \sum^{\text{4}}_{i = 1}\left\{\frac{TP_i + TN_i}{TP_i + TN_i + FP_i + FN_i}\right\} 
	}\end{equation*}
	%DIF > 
	\item \DIFadd{Micro }\DIFaddend F1-score\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{: 
	}\begin{equation*}
	\DIFadd{\text{PPV}_\mu = \sum^{\text{4}}_{i = 1}\frac{TP_i}{TP_i + FP_i}
	}\end{equation*}
	\begin{equation*}
	\DIFadd{\text{TPR}_\mu = \sum^{\text{4}}_{i = 1}\frac{TP_i}{TP_i + FN_i}
	}\end{equation*}
	\begin{equation*}
	...
	\DIFadd{\text{PPV}_M = 4 \times \sum^{\text{4}}_{i = 1}\frac{TP_i}{TP_i + FP_i}
	}\end{equation*}
	\begin{equation*}
	\DIFadd{\text{TPR}_M = 4 \times \sum^{\text{4}}_{i = 1}\frac{TP_i}{TP_i + FN_i}
	}\end{equation*}
	\begin{equation*}
...
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{table*}%DIF > [!htb]
	\DIFaddendFL \centering
	\caption{Experimental results using $k$-NN}
	\label{tab:result1}
	\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{l*9{r}}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{lrrrr*9{r}}
		\DIFaddendFL \toprule
		\multirow{2}{*}{Method} & \multirow{2}{*}{\# features} & & TPR & & & \DIFaddbeginFL & \DIFaddendFL PPV & & \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{OA}  %%%
\DIFdelendFL & \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{F1-Score} %%%
\DIFdelendFL \DIFaddbeginFL \multirow{2}{*}{AA} & \multirow{2}{*}{$\text{F1-Score}_{\mu}$} & \multirow{2}{*}{$\text{F1-Score}_M$} \DIFaddendFL \\ 
		...
		&   & \DIFdelbeginFL \DIFdelFL{Urban }%DIFDELCMD < & %%%
\DIFdelendFL Forest & \DIFaddbeginFL \DIFaddFL{Pasture }& \DIFaddendFL Ocean & Urban & Forest & \DIFaddbeginFL \DIFaddFL{Pasture }& \DIFaddendFL Ocean & \DIFaddbeginFL \DIFaddFL{Urban }\DIFaddendFL & \DIFaddbeginFL &  \DIFaddendFL \\ 
		...
		\DIFaddbeginFL \cmidrule\DIFaddFL{(lr)}{\DIFaddFL{13-13}}
		\DIFaddendFL WPE & 2 & 
		\DIFdelbeginFL \DIFdelFL{0.833 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.000 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.666  }%DIFDELCMD < & %%%
\DIFdelFL{0.666 }%DIFDELCMD < & %%%
\DIFdelendFL 0.833 & \DIFdelbeginFL \DIFdelFL{0.500  }%DIFDELCMD < & %%%
\DIFdelFL{0.800 }%DIFDELCMD < & %%%
\DIFdelFL{0.708 }%DIFDELCMD < &  %%%
\DIFdelFL{0.571  }%DIFDELCMD < \\ 
%DIFDELCMD < 		%%%
\DIFdelFL{FGPE ($\alpha$ = 1)  }%DIFDELCMD < & %%%
...
\DIFdelendFL 1.000 & 0.833 &
		...
		\DIFdelbeginFL \DIFdelFL{1.000 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.666 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.833  }%DIFDELCMD < & %%%
\DIFdelendFL 1.000 & 1.000 & 1.000 &
		\DIFdelbeginFL \DIFdelFL{0.923 }%DIFDELCMD < & %%%
\DIFdelFL{0.958 }%DIFDELCMD < &  %%%
\DIFdelFL{0.909  }%DIFDELCMD < \\ 
%DIFDELCMD < 		%%%
\DIFdelFL{AAPE (A = 1)   }%DIFDELCMD < & %%%
\DIFdelFL{2 }%DIFDELCMD < & %%%
\DIFdelendFL 1.000 & \DIFdelbeginFL \DIFdelFL{0.500  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.857 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.750  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.923 }\DIFaddendFL & 1.000 &
		\DIFdelbeginFL \DIFdelFL{0.500  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.833 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.750  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.947 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.750 }%DIFDELCMD < &  %%%
...
%DIF > \todo[inline]{Acrescentar nas conclusões que podemos usar apenas uma feature.}
...
\DIFdelbegin %DIFDELCMD < \bibliography{AbbrevRefs}
%DIFDELCMD < %%%
...
\DIFaddbegin \bibitem{Aquino2017Characterization}
\DIFadd{A.~L.~L. Aquino, H.~S. Ramos, A.~C. Frery, L.~P. Viana, T.~S.~G. Cavalcante,
  and O.~A. Rosso, ``Characterization of electric load with information theory
  quantifiers,'' }\emph{\DIFadd{Physica A}}\DIFadd{, vol. 465, pp. 277--284, 2017.
}

\bibitem{Rosso2016Signatures}
\DIFadd{O.~A. Rosso, R.~Ospina, and A.~C. Frery, ``Classification and verification of
  handwritten signatures with time causal information theory quantifiers,''
  }\emph{\DIFadd{PLoS One}}\DIFadd{, vol.~11, no.~12, p. e0166868, 2016.
}

\bibitem{Schieber2016network}
\DIFadd{T.~A. Schieber, L.~Carpi, A.~C. Frery, O.~A. Rosso, P.~M. Pardalos, and M.~G.
  Ravetti, ``Information theory perspective on network robustness,''
  }\emph{\DIFadd{Phys. Lett. A}}\DIFadd{, vol. 380, pp. 359--364, 2016.
}

\bibitem{Yue2020Gaussian}
\DIFadd{D.-X. Yue, F.~Xu, A.~C. Frery, and Y.-Q. Jin, ``A generalized }{\DIFadd{G}}\DIFadd{aussian
  coherent scatterer model for correlated }{\DIFadd{SAR}} \DIFadd{texture,'' }\emph{\DIFadd{IEEE Trans.
  Geosci. Remote Sens.}}\DIFadd{, vol.~58, no.~4, pp. 2947--2964, Apr. 2020.
}

\bibitem{Pottier2004Unsupervised}
\DIFadd{J.-S. Lee, M.~R. }{\DIFadd{Grunes}}\DIFadd{, E.~}{\DIFadd{Pottier}}\DIFadd{, and L.~}{\DIFadd{Ferro-Famil}}\DIFadd{, ``Unsupervised
  terrain classification preserving polarimetric scattering characteristics,''
  }\emph{\DIFadd{IEEE Trans. Geosci. Remote Sens.}}\DIFadd{, vol.~42, no.~4, pp. 722--731, 2004.
}

\bibitem{han2020unsupervised}
\DIFadd{P.~Han, B.~Han, X.~Lu, R.~Cong, and D.~Sun, ``Unsupervised classification for
  }{\DIFadd{P}}\DIFadd{ol}{\DIFadd{SAR}} \DIFadd{images based on multi-level feature extraction,'' }\emph{\DIFadd{Int. J.
  Remote Sens.}}\DIFadd{, vol.~41, no.~2, pp. 534--548, 2020.
}

\bibitem{huang2020classification}
\DIFadd{Z.~Huang, C.~O. Dumitru, Z.~Pan, B.~Lei, and M.~Datcu, ``Classification of
  large-scale high-resolution }{\DIFadd{SAR}} \DIFadd{images with deep transfer learning,''
  }\emph{\DIFadd{arXiv preprint arXiv:2001.01425}}\DIFadd{, 2020.
}

\bibitem{xie2020polsar}
\DIFadd{W.~Xie, G.~Ma, F.~Zhao, H.~Liu, and L.~Zhang, ``}{\DIFadd{P}}\DIFadd{ol}{\DIFadd{SAR}} \DIFadd{image classification
  via a novel semi-supervised recurrent complex-valued convolution neural
  network,'' }\emph{\DIFadd{Neurocomputing}}\DIFadd{, 2020.
}

\bibitem{liu2019task}
\DIFadd{F.~Liu, L.~Jiao, and X.~Tang, ``Task-oriented }{\DIFadd{GAN}} \DIFadd{for }{\DIFadd{P}}\DIFadd{ol}{\DIFadd{SAR}} \DIFadd{image
  classification and clustering,'' }\emph{\DIFadd{IEEE Trans. Neural Netw. Learn.
  Syst.}}\DIFadd{, vol.~30, no.~9, pp. 2707--2719, 2019.
}

\bibitem{sukawattanavijit2017ga}
\DIFadd{C.~Sukawattanavijit, J.~Chen, and H.~Zhang, ``}{\DIFadd{GA}}\DIFadd{-}{\DIFadd{SVM}} \DIFadd{algorithm for
  improving land-cover classification using }{\DIFadd{SAR}} \DIFadd{and optical remote sensing
  data,'' }\emph{\DIFadd{IEEE Geosci. Remote Sens. Lett.}}\DIFadd{, vol.~14, no.~3, pp. 284--288,
  2017.
}

\bibitem{mcnairn2014early}
\DIFadd{H.~McNairn, A.~Kross, D.~Lapen, R.~Caves, and J.~Shang, ``Early season
  monitoring of corn and soybeans with }{\DIFadd{T}}\DIFadd{erra}{\DIFadd{SAR}}\DIFadd{-}{\DIFadd{X}} \DIFadd{and }{\DIFadd{RADARSAT}}\DIFadd{-2,''
  }\emph{\DIFadd{Int. J. Appl. Earth Obs. Geoinf.}}\DIFadd{, vol.~28, pp. 252--259, 2014.
}

\bibitem{lin2017deep}
\DIFadd{Z.~Lin, K.~Ji, M.~Kang, X.~Leng, and H.~Zou, ``Deep convolutional highway unit
  network for sar target classification with limited labeled training data,''
  }\emph{\DIFadd{IEEE Geosci. Remote Sens. Lett.}}\DIFadd{, vol.~14, no.~7, pp. 1091--1095, 2017.
}

\bibitem{ressel2015neural}
\DIFadd{R.~Ressel, A.~Frost, and S.~Lehner, ``A neural network-based classification for
  sea ice types on }{\DIFadd{X}}\DIFadd{-band }{\DIFadd{SAR}} \DIFadd{images,'' }\emph{\DIFadd{IEEE J. Sel. Topics Appl.
  Earth Observ. Remote Sens.}}\DIFadd{, vol.~8, no.~7, pp. 3672--3680, 2015.
}

\bibitem{wang2019classification}
\DIFadd{R.~Wang and Y.~Wang, ``Classification of }{\DIFadd{P}}\DIFadd{ol}{\DIFadd{SAR}} \DIFadd{image using neural nonlocal
  stacked sparse autoencoders with virtual adversarial regularization,''
  }\emph{\DIFadd{Remote Sensing}}\DIFadd{, vol.~11, no.~9, p. 1038, 2019.
}

\bibitem{qin2017object}
\DIFadd{F.~Qin, J.~Guo, and W.~Sun, ``Object-oriented ensemble classification for
  polarimetric sar imagery using restricted boltzmann machines,'' }\emph{\DIFadd{Remote
  Sensing Letters}}\DIFadd{, vol.~8, no.~3, pp. 204--213, 2017.
}

\bibitem{bi2018graph}
\DIFadd{H.~Bi, J.~Sun, and Z.~Xu, ``A graph-based semisupervised deep learning model
  for }{\DIFadd{P}}\DIFadd{ol}{\DIFadd{SAR}} \DIFadd{image classification,'' }\emph{\DIFadd{IEEE Trans. Geosci. Remote
  Sens.}}\DIFadd{, vol.~57, no.~4, pp. 2116--2132, 2018.
}

\bibitem{numbisi2018multi}
\DIFadd{F.~N. Numbisi, F.~Van~Coillie, and R.~De~Wulf, ``Multi-date }{\DIFadd{S}}\DIFadd{entinel }{\DIFadd{SAR}}
  \DIFadd{image textures discriminate perennial agroforests in a tropical
  forest-savannah transition landscape,'' }\emph{\DIFadd{International Archives of the
  Photogrammetry, Remote Sensing \& Spatial Information Sciences}}\DIFadd{, vol.~42,
  no.~1, 2018.
}

\bibitem{yu2019detection}
\DIFadd{Q.~Yu, M.~Xing, X.~Liu, L.~Wang, K.~Luo, and X.~Quan, ``Detection of land use
  type using multitemporal }{\DIFadd{SAR}} \DIFadd{images,'' in }\emph{\DIFadd{IGARSS 2019 -- IEEE
  International Geoscience and Remote Sensing Symposium}}\DIFadd{.}\hskip \DIFadd{1em plus 0.5em
  minus 0.4em}\relax \DIFadd{IEEE, 2019, pp. 1534--1537.
}

\bibitem{radford2018geological}
\DIFadd{D.~D. Radford, M.~J. Cracknell, M.~J. Roach, and G.~V. Cumming, ``Geological
  mapping in }{\DIFadd{W}}\DIFadd{estern }{\DIFadd{T}}\DIFadd{asmania using radar and random forests,'' }\emph{\DIFadd{IEEE
  J. Sel. Topics Appl. Earth Observ. Remote Sens.}}\DIFadd{, vol.~11, no.~9, pp.
  3075--3087, 2018.
}

\bibitem{hagensieker2018evaluation}
\DIFadd{R.~Hagensieker and B.~Waske, ``Evaluation of multi-frequency }{\DIFadd{SAR}} \DIFadd{images for
  tropical land cover mapping,'' }\emph{\DIFadd{Remote Sensing}}\DIFadd{, vol.~10, no.~2, p. 257,
  2018.
}

\bibitem{kankaku2013alos}
\DIFadd{Y.~Kankaku, S.~Suzuki, and Y.~Osawa, ``Alos-2 mission and development status,''
  in }\emph{\DIFadd{IGARSS 2013 -- IEEE International Geoscience and Remote Sensing
  Symposium}}\DIFadd{.}\hskip \DIFadd{1em plus 0.5em minus 0.4em}\relax \DIFadd{IEEE, 2013, pp.
  2396--2399.
}

\bibitem{morena2004introduction}
\DIFadd{L.~Morena, K.~James, and J.~Beck, ``An introduction to the }{\DIFadd{RADARSAT}}\DIFadd{-2
  mission,'' }\emph{\DIFadd{Canadian Journal of Remote Sensing}}\DIFadd{, vol.~30, no.~3, pp.
  221--234, 2004.
}

\bibitem{breit2009terrasar}
\DIFadd{H.~Breit, T.~Fritz, U.~Balss, M.~Lachaise, A.~Niedermeier, and M.~Vonavka,
  ``}{\DIFadd{T}}\DIFadd{erra}{\DIFadd{SAR}}\DIFadd{-}{\DIFadd{X}} {\DIFadd{SAR}} \DIFadd{processing and products,'' }\emph{\DIFadd{IEEE Trans. Geosci.
  Remote Sens.}}\DIFadd{, vol.~48, no.~2, pp. 727--740, 2009.
}

\bibitem{storie2018urban}
\DIFadd{C.~D. Storie, ``Urban boundary mapping using }{\DIFadd{S}}\DIFadd{entinel-1}{\DIFadd{A}} {\DIFadd{SAR}} \DIFadd{data,'' in
  }\emph{\DIFadd{IGARSS 2018 -- IEEE International Geoscience and Remote Sensing
  Symposium}}\DIFadd{, 2018, pp. 2960--2963.
}

\bibitem{Florindo2012Fractal}
\DIFadd{J.~B. Florindo and O.~M. Bruno, ``Fractal descriptors based on }{\DIFadd{F}}\DIFadd{ourier
  spectrum applied to texture analysis,'' }\emph{\DIFadd{Physica A}}\DIFadd{, vol. 391, no.~20,
  pp. 4909--4922, 2012.
}

\bibitem{zhu2016antarctic}
\DIFadd{T.~Zhu, F.~Li, G.~Heygster, and S.~Zhang, ``Antarctic sea-ice classification
  based on conditional random fields from }{\DIFadd{RADARSAT}}\DIFadd{-2 dual-polarization
  satellite images,'' }\emph{\DIFadd{IEEE J. Sel. Topics Appl. Earth Observ. Remote
  Sens.}}\DIFadd{, vol.~9, no.~6, pp. 2451--2467, 2016.
}

\bibitem{dumitru2014information}
\DIFadd{C.~O. Dumitru, S.~Cui, G.~Schwarz, and M.~Datcu, ``Information content of
  very-high-resolution }{\DIFadd{SAR}} \DIFadd{images: Semantics, geospatial context, and
  ontologies,'' }\emph{\DIFadd{IEEE J. Sel. Topics Appl. Earth Observ. Remote Sens.}}\DIFadd{,
  vol.~8, no.~4, pp. 1635--1650, 2014.
}

\bibitem{akbarizadeh2012new}
\DIFadd{G.~Akbarizadeh, ``A new statistical-based kurtosis wavelet energy feature for
  texture recognition of }{\DIFadd{SAR}} \DIFadd{images,'' }\emph{\DIFadd{IEEE Trans. Geosci. Remote
  Sens.}}\DIFadd{, vol.~50, no.~11, pp. 4358--4368, 2012.
}

\bibitem{Lee1994Texture}
\DIFadd{J.-H. Lee and Y.-C. Hsueh, ``Texture classification method using multiple space
  filling curves,'' }\emph{\DIFadd{Pattern Recognit. Lett.}}\DIFadd{, vol.~15, no.~12, pp.
  1241--1244, 1994.
}

\bibitem{nguyen1982space}
\DIFadd{P.~Nguyen and J.~Quinqueton, ``Space filling curves and texture analysis,'' in
  }\emph{\DIFadd{IEEE Intl. Conf. Pattern Recognition}}\DIFadd{, 1982, pp. 282--285.
}

\bibitem{Carincotte2006changeDetection}
\DIFadd{C.~Carincotte, S.~Derrode, and S.~Bourennane, ``Unsupervised change detection
  on }{\DIFadd{SAR}} \DIFadd{images using fuzzy hidden }{\DIFadd{M}}\DIFadd{arkov chains,'' }\emph{\DIFadd{IEEE Trans.
  Geosci. Remote Sens.}}\DIFadd{, vol.~44, no.~2, pp. 432--441, Feb 2006.
}

\bibitem{Bandt2002Permutation}
\DIFadd{C.~Bandt and B.~Pompe, ``Permutation entropy: A natural complexity measure for
  time series,'' }\emph{\DIFadd{Phys. Rev. Lett.}}\DIFadd{, vol.~88, p. 174102, 2002.
}

\bibitem{Borges2019Transition}
\DIFadd{J.~Borges, H.~Ramos, R.~Mini, O.~A. Rosso, A.~C. Frery, and A.~A.~F. Loureiro,
  ``Learning and distinguishing time series dynamics via ordinal patterns
  transition graphs,'' }\emph{\DIFadd{Appl. Math. Comput.}}\DIFadd{, vol. 362, p. UNSP 124554,
  2019.
}

\bibitem{McCullough2015lagged}
\DIFadd{M.~McCullough, M.~Small, T.~Stemler, and H.~H.-C. Iu, ``Time lagged ordinal
  partition networks for capturing dynamics of continuous dynamical systems,''
  }\emph{\DIFadd{Chaos}}\DIFadd{, vol.~25, no.~5, p. 053101, 2015.
}

\bibitem{Kulp2016ordinal}
\DIFadd{C.~W. Kulp, J.~M. Chobot, H.~R. Freitas, and G.~D. Sprechini, ``Using ordinal
  partition transition networks to analyze }{\DIFadd{ECG}} \DIFadd{data,'' }\emph{\DIFadd{Chaos}}\DIFadd{, vol.~26,
  no.~7, p. 073114, 2016.
}

\bibitem{Sorrentino2015periodic}
\DIFadd{T.~Sorrentino, C.~Quintero-Quiroz, A.~Aragoneses, M.~Torrent, and C.~Masoller,
  ``Effects of periodic forcing on the temporally correlated spikes of a
  semiconductor laser with feedback,'' }\emph{\DIFadd{Opt. Express}}\DIFadd{, vol.~23, 2015.
}

\bibitem{Zhang2017ConstructingOP}
\DIFadd{J.~Zhang, J.~Zhou, M.~Tang, H.~Guo, M.~Small, and Y.~Zou, ``Constructing
  ordinal partition transition networks from multivariate time series,''
  }\emph{\DIFadd{Sci. Rep.}}\DIFadd{, vol.~7, p. 7795, 2017.
}

\bibitem{Fadlallah2013Weightedpermutation}
\DIFadd{B.~H. Fadlallah, B.~Chen, A.~Keil, and J.~C. Pr}{\DIFadd{\'i}}\DIFadd{ncipe,
  ``Weighted-permutation entropy: a complexity measure for time series
  incorporating amplitude information.'' }\emph{\DIFadd{Phys. Rev. E: Stat., Nonlinear,
  Soft Matter Phys.}}\DIFadd{, vol. 87 2, p. 022911, 2013.
}

\bibitem{xiao2009fine}
\DIFadd{L.~Xiao-Feng and W.~Yue, ``Fine-grained permutation entropy as a measure of
  natural complexity for time series,'' }\emph{\DIFadd{Chin. Phys. B}}\DIFadd{, vol.~18, no.~7,
  p. 2690, 2009.
}

\bibitem{azami2016amplitude}
\DIFadd{H.~Azami and J.~Escudero, ``Amplitude-aware permutation entropy: Illustration
  in spike detection and signal segmentation,'' }\emph{\DIFadd{Comput. Methods Programs
  Biomed.}}\DIFadd{, vol. 128, pp. 40--51, 2016.
}

\bibitem{wu2013man}
\DIFadd{W.~Wu, H.~Guo, and X.~Li, ``Man-made target detection in urban areas based on a
  new azimuth stationarity extraction method,'' }\emph{\DIFadd{IEEE Journal of Selected
  Topics in Applied Earth Observations and Remote Sensing}}\DIFadd{, vol.~6, no.~3, pp.
  1138--1146, 2013.
}

\bibitem{Lamberti2004Entropic}
\BIBentryALTinterwordspacing
\DIFadd{P.~W. Lamberti, M.~T. Mart\'in, A.~Plastino, and O.~A. Rosso, ``Intensive
  entropic non-triviality measure,'' }\emph{\DIFadd{Physica A}}\DIFadd{, vol. 334, no. 1--2, pp.
  119--131, 2004. }[\DIFadd{Online}]\DIFadd{. Available:
  }\url{http://www.sciencedirect.com/science/article/pii/S0378437103010963}
\BIBentrySTDinterwordspacing

\bibitem{martin2006generalized}
\DIFadd{M.~T. Mart\'in, A.~Plastino, and O.~A. Rosso, ``Generalized statistical
  complexity measures: Geometrical and analytical properties,'' }\emph{\DIFadd{Physica
  A}}\DIFadd{, vol. 369, no.~2, pp. 439--462, 2006.
}

\bibitem{weldon1996efficient}
\DIFadd{T.~P. Weldon, W.~E. Higgins, and D.~F. Dunn, ``Efficient }{\DIFadd{G}}\DIFadd{abor filter design
  for texture segmentation,'' }\emph{\DIFadd{Pattern Recognit.}}\DIFadd{, vol.~29, no.~12, pp.
  2005--2015, 1996.
}

\bibitem{dalal2005histograms}
\DIFadd{N.~Dalal and B.~Triggs, ``Histograms of oriented gradients for human
  detection,'' in }\emph{\DIFadd{2005 IEEE Computer Society Conference on Computer
  Vision and Pattern Recognition (CVPR'05)}}\DIFadd{, vol.~1.}\hskip \DIFadd{1em plus 0.5em minus
  0.4em}\relax \DIFadd{IEEE, 2005, pp. 886--893.
}

\bibitem{kourgli2012texture}
\DIFadd{A.~Kourgli, M.~Ouarzeddine, Y.~Oukil, and A.~Belhadj-Aissa, ``Texture modelling
  for land cover classification of fully polarimetric }{\DIFadd{SAR}} \DIFadd{images,''
  }\emph{\DIFadd{International Journal of Image and Data Fusion}}\DIFadd{, vol.~3, no.~2, pp.
  129--148, 2012.
}

\bibitem{bay2006surf}
\DIFadd{H.~Bay, T.~Tuytelaars, and L.~Van~Gool, ``Surf: Speeded up robust features,''
  in }\emph{\DIFadd{European Conference on Computer Vision}}\DIFadd{.}\hskip \DIFadd{1em plus 0.5em minus
  0.4em}\relax \DIFadd{Springer, 2006, pp. 404--417.
}

\bibitem{portnoff1980time}
\DIFadd{M.~Portnoff, ``Time-frequency representation of digital signals and systems
  based on short-time fourier analysis,'' }\emph{\DIFadd{IEEE Trans. Acoust., Speech,
  Signal Process.}}\DIFadd{, vol.~28, no.~1, pp. 55--69, 1980.
}

\bibitem{guan2019covariance}
\DIFadd{D.~Guan, D.~Xiang, X.~Tang, L.~Wang, and G.~Kuang, ``Covariance of textural
  features: A new feature descriptor for }{\DIFadd{SAR}} \DIFadd{image classification,''
  }\emph{\DIFadd{IEEE J. Sel. Topics Appl. Earth Observ. Remote Sens.}}\DIFadd{, vol.~12, no.~10,
  pp. 3932--3942, 2019.
}

\bibitem{kuhn2008building}
\DIFadd{M.~Kuhn, ``Building predictive models in }{\DIFadd{R}} \DIFadd{using the caret package,''
  }\emph{\DIFadd{J. Stat. Softw.}}\DIFadd{, vol.~28, no.~5, pp. 1--26, 2008.
}

\bibitem{mitchell1997machine}
\DIFadd{T.~M. Mitchell, }\emph{\DIFadd{Machine Learning}}\DIFadd{.}\hskip \DIFadd{1em plus 0.5em minus 0.4em}\relax
  \DIFadd{McGraw-hill New York, 1997.
}

\bibitem{TheCursesofDimensionality2018}
\DIFadd{N.~Altman and M.~Krzywinski, ``The curse(s) of dimensionality,'' }\emph{\DIFadd{Nat.
  Methods}}\DIFadd{, vol.~15, no.~6, pp. 399--400, May 2018.
}

\bibitem{ABadgingSystemforReproducibilityandReplicabilityinRemoteSensingResearch}
\DIFadd{A.~C. Frery, L.~Gomez, and A.~C. Medeiros, ``A badging system for
  reproducibility and replicability in remote sensing research,'' }\emph{\DIFadd{IEEE J.
  Sel. Topics Appl. Earth Observ. Remote Sens.}}\DIFadd{, 2020.
}

\end{thebibliography}


\DIFaddend \section*{\DIFdelbegin \DIFdel{ACKNOWLEDGEMENTS}\DIFdelend \DIFaddbegin \DIFadd{Acknowledgements}\DIFaddend }\label{ACKNOWLEDGEMENTS}
